...
RUN apt update -y && apt install build-essential cmake wget libcairo2-dev libgraphicsmagick1-dev \
libpng-dev software-properties-common -y && apt install g++-7 -y
COPY create_symlink.sh /create_symlink.sh
RUN ["chmod", "+x", "/create_symlink.sh"] && /create_symlink.sh
RUN apt-get purge cmake -y
RUN wget https://github.com/Kitware/CMake/releases/download/v3.14.3/cmake-3.14.3.tar.gz && \ 
tar xzvf cmake-3.14.3.tar.gz && cd cmake-3.14.3 
